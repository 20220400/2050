class OrderedList:
    ...
    def _bs(self, item, left = 0, right = Noned):
        if len(self._L) == 0: return False
        if right is None: right = len(self) - 1
        median = (left + right) // 2

        if self._L[median] == item: return True
        
        if (right - left) <= 1: return self._L[median] == item

        if item < self._L[median]: return self._bs(item, left, median)

        elif item > self._L[median]: return self._bs(item, median, right)
