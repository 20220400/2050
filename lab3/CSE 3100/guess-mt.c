...
void * thread_c(void * arg_in)
{
    ...
    pthread_mutex_lock(&arg->mutex);  //lock
    arg->status= S_MAX;
    ...
    pthread_mutex_unlock(&arg->mutex); //unlock 

    int holder= 1; //temporary variable to help with the while loop 
    ...
    { 
        ...
        int guessthread= arg->guess;
        pthread_mutex_unlock(&arg->mutex);

        int resultthread= gmn_check(&gmn, guessthread);
     
        arg->status= S_RESULT;
        arg->result= resultthread;
        pthread_mutex_lock(&arg->mutex); 
        if(resultthread== 0)
        {

            holder= 0; //breaks the while loop
...
void * thread_p(void *arg_in)
{
    ...
    printf("%s", arg->message);// here, the arg->message not changed after the value is set
    }
...
int main(int argc, char *argv[])
{
    ...
    pthread_create(&thread1, NULL, thread_c, &arg);
    pthread_create(&thread2, NULL, thread_p, &arg);
