...
{
    // TODO
    printer_t* argument = (printer_t*) arg;
    argument->njobs = 0;

    while( q_num_jobs(argument->jq)!= 0)
    {
        pthread_mutex_lock(&argument->jq->mutex);

        int len= q_fetch_job(argument->jq, argument->id);

        pthread_mutex_unlock(&argument->jq->mutex);
...
int main(int argc, char *argv[])
{
    ...
    for (int i=0; i<num_printers; i++)
    {
        ...
        check_pthread_return(status, "error in creating thread");
    }
    for(int j =0; j<num_printers; j++)
    {
        status = pthread_join(printers[j].thread_id, NULL);

        check_pthread_return(status, "pthread_join error");
    }
